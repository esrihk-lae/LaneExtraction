...
#from model_valid import LinkModel as LinkModel_valid		# dky: model_valid does not exist
...
#inputsat = scipy.ndimage.imread(sys.argv[1]).astype(np.float) / 255.0	# dky: orig
inputsat = imageio.imread(sys.argv[1]).astype(np.float32) / 255.0		# dky: updated for imageio

# inputseg = scipy.ndimage.imread(sys.argv[2]) / 255.0		# dky: orig
# inputseg = imageio.imread(sys.argv[2]) / 255.0			# dky: updated for imageio

#inputdirection = (scipy.ndimage.imread(sys.argv[2]).astype(np.float) - 127) / 127.0		# dky: orig
inputdirection = (imageio.imread(sys.argv[2]).astype(np.float32) - 127) / 127.0				# dky: updated for imageio

#inputgraph = pickle.load(open(sys.argv[3], "rb"))		# dky: orig
with open(sys.argv[3], "rb") as f:						# dky: modern safe python
...
#x_connector = np.zeros((1, dim[0], dim[1], 7))
...
for i in range(len(pairs)):
	if results[i] > 0.5:
		...
		seg = model.infer(x_in=image_batch[0:1,:,:,:], x_connector = connector_batch[0:1,:,:,:], context=direction_batch[0:1,:,:,:])

		Image.fromarray((seg[0][0,:,:,0]*255).astype(np.uint8))
		ok, link = seg2link(seg[0][0,:,:,0], [x1,y1], [x2,y2])
		newlink = []
		if not ok:
			#link = [[y1,x1], [y2,x2]]
			Image.fromarray((seg[0][0,:,:,0]*255).astype(np.uint8)).save(outputfolder_details+"infer_link_v4_debug_seg%d.png" % i)
			# bezier curve
