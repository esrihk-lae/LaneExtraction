...
for nid, nei in inputgraph.items():
    if len(nei) != 2:
        #print(f">>> nid={nid}, nei={nei} (len={len(nei)})")    # dky: debug
...
#print(f">>> ways: type={type(ways)},\nways={ways}")
#json.dump(ways, open(outputfolder + "/ways.json", "wt"), indent=2)
with open(outputfolder + "/ways.json", "wt") as f:
    json.dump(ways, f, indent=2, default=int)
