...
for i in 0 9 11 12 17; do
    ...
    python3 infer.py "${inputfile}" "${outputfolder}" "${model}"
    echo

    echo "Turning segmentation to graph"
    python3 segtograph/setograph.py "${outputfolder}/seg.png" 64 "${outputfolder}/graph.p"
    echo

    # extract directions and create ways.json file
    python3 infer_direction.py "${outputfolder}/direction.png" "${outputfolder}/graph.p" "${outputfolder}"
    echo

    rm -fv "../../dataset/sdmap_${i}.jpg"
done