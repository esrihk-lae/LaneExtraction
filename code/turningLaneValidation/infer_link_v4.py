...
inputsat = imageio.imread(sys.argv[1]).astype(np.float) / 255.0					# dky: updated to use imageio

# inputseg = scipy.ndimage.imread(sys.argv[2]) / 255.0					# dky: orig
# inputseg = imageio.imread(sys.argv[2]) / 255.0						# dky: updated to use imageio

# inputdirection = (scipy.ndimage.imread(sys.argv[2]).astype(np.float) - 127) / 127.0
inputdirection = (imageio.imread(sys.argv[2]).astype(np.float) - 127) / 127.0

# inputgraph = pickle.load(open(sys.argv[3]))		# dky: orig
...
modelpath = f"./model_turningLaneValidation_{cnninput}_resnet34_500ep_seg-20260818-0813/model1000"
